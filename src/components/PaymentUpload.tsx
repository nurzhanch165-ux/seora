...
export function PaymentUpload({ orderId }: { orderId: string }) {
  ...
  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.type.startsWith("image/")) {
      setError(tr("payment.imageRequired"));
      return;
    }
    if (file.size > 6 * 1024 * 1024) {
      setError(tr("payment.fileTooLarge"));
      return;
    }
    setError("");
    setUploading(true);
    ...
    setUploading(false);
    if (!res.ok) setError(res.error ?? tr("payment.uploadFailed"));
  }
    ...
    <div>
      ...
      {screenshot ? (
        ...
        >
          ...
          <span className="text-sm font-medium text-ink">{uploading ? tr("common.loading") : tr("payment.upload")}</span>
          <span className="text-xs text-muted">{tr("payment.uploadHint")}</span>
        </button>
      )}
